...
'''1970 CASES /FCL/FCL/40R4/24822.000KGS/3.133M'''
...
Find_PACKAGES = ["PKGS","CTNS","CARTON(S)","CASES","CARTONS","BAGS", "BOXES", "CRTS", "DRS", "PLTS", "ROLS", "TANKS", "PACKAGES"] # 대문자




# Find_DESCRIPTIONOFGOODS : 해당 상품 리스트 (품목이 증가 되면 추가 해줘야됨)
Find_DESCRIPTIONOFGOODS = ['FROZEN','CREAM','FUSED','CHEESE','BASSO', 'BLACK', 'BUTTER', 'CANNED', 'CHINESE', 'GROZEN', 'INDELHI', 'INSTANT', 'MUSHROOM', 'OLIVE', 'SMOKED', 'SQUID', 'WHIPPING', 'HADAY', 'MILKY', 'PEELED', 'SOY', 'SWEET', 'CREAM', 'ALIMENTARY', 'GREEN', 'CURRY', 'MILK', 'MOZZARELLA', 'DANG', 'RENNET', 'SUNLEE', 'NATURAL', "IT'S", 'CORN', 'PASTAVILLA', 'UNSALTED', 'WELL', 'HNT', 'SALT', 'DAIRY PRODUCTS']
...
for Find_Value in split_list: # 이중루프로 리스트 마다 해당하는 값을 찾음
    ...
    for PACKGES in Find_PACKAGES:   # Find_PACKAGES에 해당하는 값이 포함되어 있으면 PACKGES에 포함된 값 저장
        ...
        # if 'FCL', 'LCL'가 Find_Value 줄에 값이 존재 하지 않을 시 루프 Continue
        if 'FCL' not in Find_Value and 'LCL' not in Find_Value:
            continue
        ...
        if DESCRIPTIONOFGOODS in Find_Value:    # Find_DESCRIPTIONOFGOODS에 해당 값이 같으면 DESCRIPTIONOFGOODS에 값 저장
            ...
            #     DESCRIPTIONOFGOODS_list.append(Find_Value)
                for EXCEPT_word in EXCEPT_LIST: # 제외 키워드 확인 후 값 해당하는 값 지우기
                    ...
                    if EXCEPT_word in Find_Value: # Find_Value에 제외키워드가 포함시 제거
...
# DESCRIPTIONOFGOODS_list = list(set(DESCRIPTIONOFGOODS_list))
