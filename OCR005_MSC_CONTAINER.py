...
inputOcr = '''1 FIexibag of 100% REFINED BLEACHED DEODORIZED WINTERIZED SUNFLOWER
SEED
OIL,NO ADDITIVES ; Customs Code 150930
Net weight 21480.00 KGM
...
def Sub1(find_row_text, list_CONTAINERNO, list_SEALNO, list_CONTAINER_TYPE, before_find_row_text):
    ...
    # 정규식 패턴(2)
    p_CONTAINERNO = r"[A-Z]{4}\s?[A-Z]?[0-9]{6}[-]?[0-9]{1}" # WHSU6111902(문자4, 숫자7), 숫자포함 0개 또는 1개이며 대문자 영어 4개이고 공백0개 또는 1개이며,대문자 영어 0개이상 포함하며 숫자는 7개
    ...
    if 'SEAL' in before_find_row_text or 'L/C' in before_find_row_text or 'SEAL' in find_row_text:
        ...
        #         list_SEALNO.extend(t_SEALNO)
            ...
            # match에 값이 존재 할 경우
            if 'L/C' in find_row_text or 'LC N' in find_row_text or 'NET WEIGHT' in find_row_text:
                    pass
            elif P_match:
                SealNo_bool = True
    ...
    return list_CONTAINERNO, list_SEALNO, list_CONTAINER_TYPE, before_find_row_text
...
def Sub2(find_row_text, list_DESCRIPTIONOFGOODS, list_PACKAGES):
    ...
    Find_PACKAGES = ["PKGS","CTNS","CARTON(S)","CASES","CARTONS","BAGS", "BOXES", "CRTS", "DRS", "PLTS", "ROLS", "TANKS", "PACKAGE"] # 대문자

    # Find_DESCRIPTIONOFGOODS : ★ 해당 상품 리스트 (품목이 증가 되면 추가 해줘야됨) ★
    Find_DESCRIPTIONOFGOODS = ['FROZEN','CREAM','FUSED','CHEESE','BASSO', 'BLACK', 'BUTTER', 'CANNED', 'CHINESE', 'GROZEN', 'INDELHI', 'INSTANT', 'MUSHROOM', 'OLIVE', 'SMOKED', 'SQUID', 'WHIPPING', 'HADAY', 'MILKY', 'PEELED', 'SOY', 'SWEET', 'CREAM', 'ALIMENTARY', 'GREEN', 'CURRY', 'MILK', 'MOZZARELLA', 'DANG', 'RENNET', 'SUNLEE', "IT'S", 'CORN', 'PASTAVILLA', 'UNSALTED', 'WELL', 'HNT', 'SALT', 'FROZEN BONELESS BEEF-20C', 'FROZEN PORK', 'FROZEN MANGOES', 'RENNET CASEIN', 'FROZEN FRENCH FRIES', 'BASSO POMACE OLIVE OIL', 'FROZEN SALMON COHO HG','SUNFLOWER']
        ...
        if DESCRIPTIONOFGOODS in find_row_text:    # Find_DESCRIPTIONOFGOODS에 해당 값이 같으면 DESCRIPTIONOFGOODS에 값 저장
            ...
            for EXCEPT_word in EXCEPT_LIST: # 제외 키워드 확인 후 값 해당하는 값 지우기
                ...
                if EXCEPT_word in find_row_text: # find_row_text 제외키워드가 포함시 제거
                    ...
                    find_row_text = re.sub("\d*[.,]{1,1}\w*", '', find_row_text) # 뒤에 영어가 있고 ".", ","가 1개가 포함된 숫자가 있으면 제거
...
def Main(inputOcr):
    ...
    for find_row_text in inputOcr:
        ...
        Sub1_Result = Sub1(find_row_text, list_CONTAINERNO, list_SEALNO, list_CONTAINER_TYPE, before_find_row_text)
