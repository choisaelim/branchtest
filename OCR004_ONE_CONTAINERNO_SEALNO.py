...
def test(inputOcr):
    ...
    # ■■■■■■■ CONTAINERNO 에서 나온 결과값을 쉼표로 분리하여 각 요소마다 SEALNO에 "0"을 추가하는 로직 ■■■■■■
    # 추후 SEALNO 가 인식이 잘 되었을떄 이부분 제외 후 주석처리 된 내용 다 활성화 필요

    # "," 기준 각 요소마다 MEASUREMENT에 "0"을 추가
    SEALNO_count = ["0" for sealno in val_CONTAINERNO]

    # 딕셔너리 선언
    # return_value['CONTAINERNO'] = val_CONTAINERNO.replace(" ","")
    return_value['CONTAINERNO'] = val_CONTAINERNO
    return_value['SEALNO'] = SEALNO_count
