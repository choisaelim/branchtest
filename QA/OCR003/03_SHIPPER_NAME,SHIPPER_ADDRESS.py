...
FIRST_SHIPPER_ADDR_LINE = ["RIDDER", "GHELLINCKSTRAAT"]

#GHELLINCKSTRAAT 길이
gh_lenth = len(FIRST_SHIPPER_ADDR_LINE[1])
    ...
    for row in inputOcr_result:
        ...
        if len(row) > 0:
            if SHIP_NAME == '':
                ...
                if addr_value == '':
                    addr_value = row
                    #Shipper address 첫 줄 키워드가 있으면 첫 줄만 읽고 중지
                    if any(text in row for text in FIRST_SHIPPER_ADDR_LINE) :
                        # GHELLINCKSTRAAT 포함하면 해당 글자까지 읽고 반환한다(뒤의 숫자 9 뺌)
                        addr_value = row[:row.find(FIRST_SHIPPER_ADDR_LINE[1]) + gh_lenth]
